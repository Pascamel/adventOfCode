...
export class Day4 implements IDay<number[]> {
  ...
  getWord(row:number, col:number, rowIncrement:number, colIncrement:number, length:number) {
    let result = '';
  ...
  getCross(row:number, col:number) {
    return this.getWord(row-1, col-1, 1, 1, 3)+ this.getWord(row-1, col+1, 1, -1, 3);
  }
  ...
  solve(fileName: string) {
    ...
    const step1 = file
      .reduce((accRow, v, row) => accRow + v.split('')
      .reduce((acc, _v, col) => acc         
