...
export class Day09 implements IDay<number[]> {
  ...
  step1(file: number[]) {
    const files: number[] = file.reduce(
      (acc, n, index) => [
        ...acc,
        ...new Array(n).fill(index % 2 === 0 ? index / 2 : -1),
      ],
      [] as number[]
    );
  ...
  step2(file: number[]) {
    ...
    while (tail > 0) {
      ...
      const firstSpaceStart = spaces
        .filter((s) => s.length >= length)
        .map(({ start }) => start)
        .reduce((acc, v) => Math.min(acc, v), Number.MAX_VALUE);
      const firstSpaceIndex = spaces.findIndex(
        ({ start }) => start === firstSpaceStart
      );
  ...
  run() {
    const [step1, step2] = this.solve('data/Day09.sample');
