...
export class Day11 implements IDay { 
  ...
  solve(input: string) {
    const sequence = /(?=abc|bcd|cde|def|efg|fgh|ghi|hij|ijk|jkl|klm|lmn|mno|nop|opq|pqr|qrs|rst|stu|tuv|uvw|vwx|wxy|xyz)/;
    const banned = /^[^iol]*$/
    const pairs = /([a-z])\1.*([a-z])\2/;

    let result1 = input;    

    while (!sequence.test(result1) || !banned.test(result1) || !pairs.test(result1)) {
      result1 = this.increment(result1);
    }

    let result2 = this.increment(result1);

    while (!sequence.test(result2) || !banned.test(result2) || !pairs.test(result2)) {
  ...
  run() {
    const [step1, step2] = this.solve('vzbxkghb');
